...
# --- NEW TEST ---
echo ""
echo ">>> Triggering connection 4 (Custom Certificate)..."

mitmdump -q -s client_audit_certificates.py \
  --certs *=./cert.pem &
MITM_PID=$!

sleep 3

curl --proxy http://127.0.0.1:8080 https://www.google.com -s -o /dev/null
